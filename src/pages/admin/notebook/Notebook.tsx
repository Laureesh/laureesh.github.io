...
import { DEFAULT_NOTEBOOK_PREFERENCES, NOTEBOOK_PREFERENCES_KEY, readNotebookPreferences, type NotebookPreferences } from "./notebookPreferences";
...
export default function Notebook() {
  ...
  const [noteSort, setNoteSort] = useState<NoteSort>(() => {
    try { const saved = localStorage.getItem(`${STORAGE_KEY}.sort`); return SORT_OPTIONS.find(([key]) => key === saved)?.[0] ?? "updated"; } catch { return "updated"; }
  });
  const [pinnedFirst, setPinnedFirst] = useState(() => { try { return localStorage.getItem(`${STORAGE_KEY}.pinnedFirst`) !== "false"; } catch { return true; } });
  ...
  useEffect(() => {
    ...
    if (route.kind === "note" && data.notes.some((note) => note.id === route.id)) {
      ...
      const note = data.notes.find((item) => item.id === route.id)!;
      const view = new URLSearchParams(location.search).get("view");
      setFolderFilter(view && (["all", "pinned", "archive"].includes(view) || data.folders.some((folder) => folder.id === view)) ? view : note.archived ? "archive" : note.folderId ?? "all");
  ...
  }, [data.folders, data.notes, location.pathname, location.search]);

  useEffect(() => {
    if (!preferences.automaticHistory) return;
    ...
    }, preferences.historyMinutes * 60_000);
  ...
  useEffect(() => {
    try { localStorage.setItem(`${STORAGE_KEY}.sort`, noteSort); localStorage.setItem(`${STORAGE_KEY}.pinnedFirst`, String(pinnedFirst)); } catch { /* preferences are optional */ }
  }, [noteSort, pinnedFirst]);

  const updateNoteById = (noteId: string, changes: Partial<Note>) => setData((current) => ({ ...current, notes: current.notes.map((note) => note.id === noteId ? { ...note, ...changes, updatedAt: now() } : note) }));
  ...
  const createNote = (folderId: string | null = typeof folderFilter === "string" && !['all', 'pinned', 'archive'].includes(folderFilter) ? folderFilter : null) => {
    const clipboardChanges = preferences.readClipboard ? readClipboardNote(clipboardDestination) : Promise.resolve(null);
    const note: Note = { id: id("note"), title: preferences.defaultTitle.trim() || "Untitled note", html: "<p><br></p>", folderId, tags: tagFilter ? [tagFilter] : [], pinned: folderFilter === "pinned", archived: folderFilter === "archive", noteDate: localDateKey(), createdAt: now(), updatedAt: now(), attachments: [], versions: [] };
  ...
  const deleteFolder = (folder: Folder) => {
    ...
    if (folderFilter === folder.id) openFolder("all");
  };
  const deleteNote = (note = selected) => {
    if (!note || !confirm(`Delete “${note.title}”? This cannot be undone.`)) return;
    setData((current) => ({ ...current, notes: current.notes.filter((item) => item.id !== note.id), deletedNoteIds: { ...(current.deletedNoteIds ?? {}), [note.id]: now() } }));
    if (note.id === selectedId) {
      const next = visibleNotes.find((item) => item.id !== note.id);
      setSelectedId(next?.id ?? "");
      if (next) openNote(next.id); else openFolder(folderFilter);
    }
  };
  const duplicateNote = (note = selected) => {
    if (!note) return;
    const copy: Note = { ...note, id: id("note"), title: `${note.title} (copy)`, tags: [...note.tags], createdAt: now(), updatedAt: now(), versions: [], attachments: note.attachments.map((file) => ({ ...file, id: id("file") })) };
    setData((current) => ({ ...current, notes: [copy, ...current.notes] })); setQuery(""); openNote(copy.id);
  };
  ...
  const stepCircleIndex = (event: ReactMouseEvent<HTMLDivElement>) => {
    const list = event.currentTarget.querySelector("ol.notebook-steps");
    if (!list) return -1;
    return [...list.children].findIndex(step => {
      // The circle is a CSS pseudo-element, so only its bounds toggle completion.
      const rect = step.getBoundingClientRect();
      const circle = getComputedStyle(step, "::before");
      const left = rect.left + parseFloat(circle.left);
      const top = rect.top + parseFloat(circle.top);
      return event.clientX >= left && event.clientX <= left + parseFloat(circle.width)
        && event.clientY >= top && event.clientY <= top + parseFloat(circle.height);
    });
  };
  const handleStepCircleClick = (event: ReactMouseEvent<HTMLDivElement>) => {
    const index = stepCircleIndex(event);
    if (index < 0) return;
    event.preventDefault();
    changeStep({ type: "complete", index });
  };
  ...
  const handleEditorKeyDown = (event: ReactKeyboardEvent<HTMLDivElement>) => {
    if (event.key === "Backspace" && !event.altKey && !event.ctrlKey && !event.metaKey && !event.nativeEvent.isComposing && mergeStepBackward(event.currentTarget)) {
      event.preventDefault();
      updateNote({ html: event.currentTarget.innerHTML });
      return;
    }
    if (event.key === "Enter" && !event.shiftKey && !event.altKey && !event.ctrlKey && !event.metaKey && !event.nativeEvent.isComposing && (splitStepAtHeading(event.currentTarget) || exitInstructionList(event.currentTarget, true))) {
      event.preventDefault();
      updateNote({ html: event.currentTarget.innerHTML });
      return;
    }
  ...
  const importBackup = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try { const value = normalizeNotebook(JSON.parse(await file.text())); if (!value) throw new Error("This is not a valid Notebook backup."); if (!confirm(`Restore ${value.notes.length} notes and ${value.folders.length} folders? This replaces the current notebook.`)) return; setData(value); setSelectedId(value.notes[0]?.id ?? ""); openFolder("all"); }
  ...
  const openHelp = () => navigate(`${NOTEBOOK_BASE}/help?returnTo=${encodeURIComponent(location.pathname + location.search)}`);
  ...
  const changePreferences = (changes: Partial<NotebookPreferences>) => {
    const next = { ...preferences, ...changes };
    setPreferences(next);
    if (changes.defaultView) setListMode(changes.defaultView);
    try { localStorage.setItem(NOTEBOOK_PREFERENCES_KEY, JSON.stringify(next)); setSettingsStorageError(false); }
    catch { setSettingsStorageError(true); }
  };
  const resetPreferences = () => {
    changePreferences({ ...DEFAULT_NOTEBOOK_PREFERENCES });
    changeClipboardDestination("title");
    setNoteSort("updated");
    setPinnedFirst(true);
  };
  const utilityPage = notebookRoute(location.pathname).kind;
  if (utilityPage === "settings" || utilityPage === "help") {
    ...
    if (utilityPage === "help") return <NotebookHelp returnTo={returnTo} />;
    return <NotebookSettings destination={clipboardDestination} onChange={changeClipboardDestination} returnTo={returnTo} storageError={settingsStorageError} preferences={preferences} onPreferencesChange={changePreferences} sort={noteSort} onSortChange={setNoteSort} pinnedFirst={pinnedFirst} onPinnedFirstChange={setPinnedFirst} onReset={resetPreferences} onBackup={() => download("notebook-backup.json", JSON.stringify(data, null, 2), "application/json")} />;
  }
  if (!selected) return <main className="notebook-empty"><div><span>▱</span><h1>Your notebook is empty</h1><p>Create a note to start writing. Everything will autosave.</p><button onClick={() => createNote()}>＋ Create your first note</button><label>Restore a backup<input type="file" accept="application/json,.json" onChange={importBackup} /></label><button onClick={openSettings}>Settings</button><button onClick={openHelp}>Help</button><button className="quiet" onClick={() => navigate("/admin-dashboard/private-pages")}>← Private pages</button></div></main>;
  ...
  return <div className={`notebook-shell ${sidebarOpen ? "" : "sidebar-hidden"}`}>
    {contextMenu && contextNote && <NoteActionsMenu x={contextMenu.x} y={contextMenu.y} title={contextNote.title} folderId={contextNote.folderId} folders={data.folders} onClose={() => setContextMenu(null)} onMove={folderId => updateNoteById(contextNote.id, { folderId })} actions={[
      { label: "View note", run: () => openNote(contextNote.id) },
      { label: "Edit note", run: () => { openNote(contextNote.id); requestAnimationFrame(() => editorRef.current?.focus()); } },
      { label: "Rename", run: () => { const title = prompt("Note title", contextNote.title); if (title?.trim()) updateNoteById(contextNote.id, { title: title.trim().slice(0, 180) }); } },
      { label: "Duplicate", run: () => duplicateNote(contextNote) },
      { label: "New note in this folder", run: () => createNote(contextNote.folderId) },
      { label: contextNote.pinned ? "Unpin" : "Pin", run: () => updateNoteById(contextNote.id, { pinned: !contextNote.pinned }) },
    ...
    <main className={`notebook-editor ${mobileEditorOpen ? "mobile-open" : ""}`}>
      ...
      <div className="editor-toolbar" role="toolbar" aria-label="Text formatting"><button title="Undo" onClick={() => format("undo")}>↶ Undo</button><button title="Redo" onClick={() => format("redo")}>↷ Redo</button><i /><button title="Large heading" onClick={() => format("formatBlock", "h1")}>Heading 1</button><button title="Medium heading" onClick={() => format("formatBlock", "h2")}>Heading 2</button><button title="Bold" onClick={() => format("bold")}><b>Bold</b></button><button title="Italic" onClick={() => format("italic")}><i>Italic</i></button><TextColorControl key={selected.id} editorRef={editorRef} onChange={html => updateNote({ html })} /><button title="Highlight selected text" onClick={() => format("hiliteColor", "#fff09a")}>Highlight</button><button title="Bulleted list" onClick={() => format("insertUnorderedList")}>• Bullets</button><button title="Exit the current list and add an unnumbered line" onMouseDown={event => event.preventDefault()} onClick={() => { if (editorRef.current && exitInstructionList(editorRef.current)) { editorRef.current.focus(); updateNote({ html: editorRef.current.innerHTML }); } }}>Exit list</button><button title="Numbered list" onClick={() => format("insertOrderedList")}>1. Numbers</button><button title="Arrange note headings or a numbered list as a step-by-step timeline" aria-pressed={selected.html.includes('class="notebook-steps"')} onClick={toggleStepLayout}>Steps layout</button><button title="Code block" onClick={() => format("formatBlock", "pre")}>Code</button><button title="Add a link" onClick={() => { const url = prompt("Paste a link URL"); if (url) format("createLink", url); }}>Link</button><button title="Type using your voice" onClick={startVoice}>🎙 Dictate</button><label className="attach-button" title="Attach images, PDFs, audio, or video">＋ Attach files<input type="file" multiple accept="image/*,application/pdf,audio/*,video/*" onChange={attachFiles} /></label></div>
      ...
      <div ref={editorRef} className="rich-editor" style={{ fontSize: preferences.fontSize }} contentEditable suppressContentEditableWarning onMouseDown={event => { if (stepCircleIndex(event) >= 0) event.preventDefault(); }} onClick={handleStepCircleClick} onBeforeInput={event => { const input = event.nativeEvent as InputEvent; if (!input.isComposing && ((input.inputType === "deleteContentBackward" && mergeStepBackward(event.currentTarget)) || (input.inputType === "insertParagraph" && (splitStepAtHeading(event.currentTarget) || exitInstructionList(event.currentTarget, true))))) { event.preventDefault(); updateNote({ html: event.currentTarget.innerHTML }); } }} onKeyDown={handleEditorKeyDown} onPaste={pasteScreenshots} onInput={event => updateNote({ html: event.currentTarget.innerHTML })} data-placeholder="Start writing… Paste a screenshot here to attach it." />
