...
import StepCircleButtons from "./StepCircleButtons";
...
// The header can wrap as the editor narrows or its status text changes.
function measureEditorHeader(header: HTMLElement | null) {
  const editor = header?.parentElement;
  if (!header || !editor) return;
  const updateHeight = () => editor.style.setProperty("--editor-header-height", `${header.getBoundingClientRect().height}px`);
  updateHeight();
...
export default function Notebook() {
  ...
  const changeStep = (change: StepChange) => {
    ...
    const before = editorRef.current?.innerHTML ?? selected.html;
    const html = changeStepLayout(before, change);
    if (html === before) return;
    if (editorRef.current) editorRef.current.innerHTML = html;
    setStepUndo({ noteId: selected.id, before, after: html });
  ...
  return <div className={`notebook-shell ${sidebarOpen ? "" : "sidebar-hidden"}`}>
    ...
    <main className={`notebook-editor ${mobileEditorOpen ? "mobile-open" : ""}`}>
      ...
      <div ref={editorRef} className="rich-editor" style={{ fontSize: preferences.fontSize }} contentEditable suppressContentEditableWarning onBeforeInput={event => { const input = event.nativeEvent as InputEvent; if (!input.isComposing && ((input.inputType === "deleteContentBackward" && mergeStepBackward(event.currentTarget)) || (input.inputType === "insertParagraph" && (splitStepAtHeading(event.currentTarget) || exitInstructionList(event.currentTarget, true))))) { event.preventDefault(); updateNote({ html: event.currentTarget.innerHTML }); } }} onKeyDown={handleEditorKeyDown} onPaste={pasteScreenshots} onInput={event => updateNote({ html: event.currentTarget.innerHTML })} data-placeholder="Start writing… Paste a screenshot here to attach it." />
      <CodeCopyButtons key={`copy-${selected.id}`} editorRef={editorRef} />
      <StepCircleButtons key={selected.id} editorRef={editorRef} onToggle={index => changeStep({ type: "complete", index })} />
      </div>
